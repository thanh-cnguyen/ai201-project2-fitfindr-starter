...
1. Call `search_listings` with the user's query parameters (description, size, max_price).
   1. If `search_listings` returns an empty list:
      1. If the original query included `size` or `max_price`, retry once with loosened criteria by keeping the same `description` but setting `size=None` and `max_price=None`.
      2. If the retry returns results, store those results in `session["search_results"]`, set `session["retry_attempted"] = True`, and store a message explaining that the agent broadened the search.
      3. If the retry also returns no results, set `session["error"]` and stop. The agent should not call `suggest_outfit` without a selected item.
...
- After parsing the user query, the agent stores `description`, `size`, and `max_price` in `session["parsed"]`. After `search_listings`, the returned list is stored in `session["search_results"]`; if the list is empty, the agent retries with loosened criteria. If the retry also returns no results, the agent sets `session["error"]` and returns early. Otherwise, if the retry finds results, the agent sets `session["retry_attempted"] = True` and stores an explanatory message to `session["retry_message"]`. If results exist, the first/top-ranked listing is stored in `session["selected_item"]` and passed into `suggest_outfit`.
...
| --------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| search_listings | No results match the query            | Retry once with loosened criteria by removing `size` and `max_price` while keeping the same description. If the retry finds results, continue with the top retry result and tell the user the search was broadened. If the retry also returns no results, stop before `suggest_outfit` and suggest broader keywords, removing filters, or increasing budget. |
...
flowchart TD
    ...
    D --> |no results and filters exist| X[Retry search without size and price filters]
    X --> |retry results found| E
    E --> S
    X --> |retry no results| L[Store error and stop]
    D --> |no results and no filters to loosen| L
    L --> S
    S --> F[Select top item]
    ...
    H --> |empty wardrobe| M[Store general styling advice]
    M --> S
    I --> S
...
- _AI Tools:_ Copilot for coding each tool, using the specific tool specs outlined above as input; ChatGPT for code review and testing guidance for each tool, using the tool specs and error handling sections as input.
- _Provided Input:_ For each tool, provide the relevant section from this planning.md that describes the tool's functionality, input parameters, return values, and failure modes. Also provide the agent diagram to show how the tool fits into the overall architecture.
- _Expected Output:_ Implemented functions for each tool that adhere to the specifications and handle the described failure modes.
- _Verification:_
...
- _AI Tools:_ Copilot for coding the planning loop and state management, using the overall architecture and tool specifications as input; ChatGPT for code review and testing guidance for the planning loop, using the architecture, planning loop logic, state management, and error handling sections as input.
- _Provided Input:_ The overall architecture diagram and the tool specifications from this planning.md.
- _Expected Output:_ Implemented planning loop and state management that correctly orchestrates tool calls and handles state transitions.
- _Verification:_
